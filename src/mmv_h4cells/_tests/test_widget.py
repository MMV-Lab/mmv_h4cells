...
def test_update_labels(create_widget, params):
    ...
    widget.lineedit_conversion_rate.setText(params[2])
    widget.combobox_conversion_unit.setCurrentIndex(params[3])
    ...
    assert (
        widget.lineedit_conversion_rate.text() == params[2]
        if params[2]
        else "1"
    )
    assert (
    ...
    unit = ["mm", "µm", "nm"][params[3]] + "²" if params[2] else "pixel"
    factor = float(params[2]) if params[2] else 1
    assert widget.label_mean_included.text() == str(
        f"{str(params[1] * factor)} {unit}"
    )
    assert widget.label_std_included.text() == str(
        f"{str(params[1] * factor)} {unit}"
    )
...
@pytest.mark.parametrize("conversion_rate", ["", "2", "2.5"])
...
def test_export(
    ...
    widget.lineedit_conversion_rate.setText(conversion_rate)
    widget.combobox_conversion_unit.setCurrentIndex(unit)
    ...
    mock_write.assert_has_calls(
        [
            call(
                Path("test.csv"),
                widget.metric_data,
                (widget.mean_size, widget.std_size),
                (
...
def test_include_on_click_overlap(
    mock_display, mock_include, mock_check, create_started_widget, remaining
):
    widget = create_started_widget
    widget.remaining = remaining.copy()
...
def test_exclude_on_click(
    mock_display, mock_update, create_started_widget, remaining
):
    widget = create_started_widget
    widget.remaining = remaining.copy()
    widget.exclude_on_click()
...
@pytest.mark.parametrize(
    "ids", [None, [1], [2], [3], [1, 2], [1, 3], [2, 3], [1, 2, 3]]
)
def test_include_multiple_on_click(
    ...
    assert widget.lineedit_next_id.text() == str(min(widget.remaining))
    mock_display_next.assert_called_once_with(False)
    text = ""
    if 1 in ids:
...
def test_include_multiple(mock_include, create_widget_in_analysis, ids):
    widget = create_widget_in_analysis
    included, ignored, overlapped = widget.include_multiple(ids)
...
@pytest.mark.parametrize("btn_text", ["Draw own cell", "Confirm/Back"])
def test_draw_own_cell(create_started_widget, btn_text):
    ...
    with patch.object(widget, "include_on_click") as mock_include:
        ...
        if btn_text == "Draw own cell":
            assert widget.btn_segment.text() == "Confirm/Back"
        ...
        if last_evaluated_id == 1 or not check_lowered:
            if 10 in remaining:
                ...
                try:
                    ...
                    raise AssertionError(
                        f"Expected QMessageBox.exec_ to be called once, but was called {mock_exec.call_count} times."
                    )
                try:
                    mock_set_text.assert_called_once_with(
                        "Given id is not in remaining cells."
                    )
                except AssertionError:
                    raise AssertionError(
                        f"Expected QMessageBox.setText to be called once with 'Given id is not in remaining cells.', but was called {mock_set_text.call_count} times with {mock_set_text.call_args_list}."
                    )
                try:
            ...
            if 10 in remaining:
                ...
                ), f"Expected QMessageBox.exec_ to be called twice, but was called {mock_exec.call_count} times."
                assert mock_set_text.called_with(
                    "Given id is not in remaining cells."
                )
            if 21 in remaining:
                try:
                    ...
                    raise AssertionError(
                        f"Expected QMessageBox.setText to be called once with 'Lowering the next cell id is a bad idea.', but was called {mock_set_text.call_count} times with {mock_display_cell.call_args_list}."
                    )
            else:
                try:
                    mock_set_text.assert_called_with(
                    ...
                    raise AssertionError(
                        f"Expected QMessageBox.setText to be called once with 'Dataset is finished. Jumping to earlier cells.', but was called {mock_set_text.call_count} times with {mock_display_cell.call_args_list}."
                    )
...
def test_handle_overlap(
    ...
    with patch.object(
        ...
        assert mock_exec.called_once()
    assert mock_set_text.called_once_with(
        "Overlap detected and highlighted. Please remove the overlap!"
    )
    if user_drawn:
        assert mock_set_standard_buttons.called_once_with(
            QMessageBox.Ok | QMessageBox.Cancel
        )
        assert mock_set_default_button.called_once_with(QMessageBox.Cancel)
